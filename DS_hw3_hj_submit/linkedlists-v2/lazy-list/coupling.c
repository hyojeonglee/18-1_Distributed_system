...
int lockc_insert(intset_l_t *set, val_t val, int *abort_count) {
	node_l_t *curr, *next, *newnode;
	int found;
	
	HLE_LOCK(&set->head->lock, abort_count);
//	printf("abort_count is %d\n", *abort_count);
	curr = set->head;
	HLE_LOCK(&curr->next->lock, abort_count);
	next = curr->next;
	
	while (next->val < val) {
		
		HLE_UNLOCK(&curr->lock);
		curr = next;
		HLE_LOCK(&next->next->lock, abort_count);
		next = curr->next;
